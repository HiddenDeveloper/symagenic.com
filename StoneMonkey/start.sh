...
if [ "$DOCKER_MODE" = true ]; then
  echo -e "${BLUE}🐳 Mode: Full Docker Deployment (ailumina-server + infrastructure)${NC}"
else
  echo -e "${BLUE}💻 Mode: Local Development (local server + Docker infrastructure)${NC}"
fi
...
if [ "$DOCKER_MODE" = true ]; then
  # Docker mode summary
  echo -e "${BLUE}🐳 Docker Mode - All services running in containers${NC}"
  echo ""
  ...
  if [ ! -z "$CODESPACES" ]; then
    ...
    echo -e "  • AIlumina UI:        ${GREEN}${UI_URL}${NC}"
    echo -e "  • Neo4j Browser:      Check Codespaces ports tab for port 7474"
    echo -e "  • Qdrant Dashboard:   Check Codespaces ports tab for port 6333"
  else
  ...
  echo ""
  echo -e "  • All MCP servers running in Docker containers"
  ...
  # Local mode summary
  echo -e "${BLUE}💻 Local Mode - Server ready to start${NC}"
  echo ""
