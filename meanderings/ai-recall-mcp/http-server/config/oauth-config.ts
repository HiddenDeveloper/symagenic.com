...
export const RECALL_OAUTH_CONFIG = {
   ...
   */
  oauthIssuer: process.env['RECALL_OAUTH_ISSUER'] || process.env['MEMORY_RESOURCE_ID'] || "http://localhost:3003",
   ...
   */
  authorizationServers: process.env['RECALL_AUTH_SERVERS']?.split(",").filter(Boolean) || [],

  /**
   * Supported OAuth scopes for this MCP server (recall only)
   * - read:recall: Query conversation history, semantic search
   */
  supportedScopes: (process.env['RECALL_OAUTH_SCOPES'] || "read:recall")
    .split(",")
    .filter(Boolean),
  ...
  jwt: {
     ...
     */
    clockTolerance: parseInt(process.env['RECALL_JWT_CLOCK_TOLERANCE'] || "30", 10),

    /**
     * JWKS cache TTL in seconds
     * How long to cache authorization server's public keys
     */
    jwksCacheTtl: parseInt(process.env['RECALL_JWKS_CACHE_TTL'] || "3600", 10),
     ...
     */
    tokenCacheTtl: parseInt(process.env['RECALL_TOKEN_CACHE_TTL'] || "300", 10),
  },
...
export function validateOAuthConfig(): { valid: boolean; errors: string[] } {
  ...
  if (RECALL_OAUTH_CONFIG.enabled) {
    if (!RECALL_OAUTH_CONFIG.resourceId) {
      errors.push("RECALL_RESOURCE_ID is required when OAuth is enabled");
    }

    if (RECALL_OAUTH_CONFIG.authorizationServers.length === 0) {
      errors.push("At least one authorization server (RECALL_AUTH_SERVERS) is required when OAuth is enabled");
    }

    // Validate that resourceId is a valid URL
    try {
      new URL(RECALL_OAUTH_CONFIG.resourceId);
    } catch {
      errors.push(`RECALL_RESOURCE_ID must be a valid URL, got: ${RECALL_OAUTH_CONFIG.resourceId}`);
    }

    // Validate authorization server URLs
    for (const serverUrl of RECALL_OAUTH_CONFIG.authorizationServers) {
      try {
        new URL(serverUrl);
      } catch {
