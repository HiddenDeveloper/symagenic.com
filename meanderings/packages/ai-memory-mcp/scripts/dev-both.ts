...
/**
 * Start both Memory HTTP server and STDIO wrapper in development mode
 */
...
async function startBothDev() {
  console.log('🧠 Starting Memory dual-transport system (development mode)...');
  
  // Start HTTP server with tsx
  console.log('📡 Starting Memory HTTP server (dev)...');
  const httpProcess = spawn('npx', ['tsx', join(projectRoot, 'http-server', 'index.ts')], {
    cwd: projectRoot,
  ...
  console.log('🔄 Starting Memory STDIO wrapper (dev)...');

  // Start STDIO wrapper with tsx
  const stdioProcess = spawn('npx', ['tsx', join(projectRoot, 'stdio-wrapper', 'index.ts')], {
    cwd: projectRoot,
    stdio: ['pipe', 'pipe', 'pipe'],
    env: { ...process.env }
  });
